			...
			if (this._state == State.RUNNING) {
				...
				try {
					...
					if (
						...
						Logging.showLogError(`Block ${block.number} anchor ${roundAnchor}`)
						roundAnchor = undefined
					}
					// else Logging.showError(`Block ${block.number} anchor ${roundAnchor}`)
				} catch (e) {
					Logging.showLogError(`currentRoundAnchor: ${e}`)
					roundAnchor = undefined
				}
				try {
					const paused = await this.redistribution.paused()
					if (paused) {
						Logging.showLogError(`Redistribution.paused() = ${paused}`)
					}
				} catch (e) {
					Logging.showLogError(`paused: ${e}`)
				}
